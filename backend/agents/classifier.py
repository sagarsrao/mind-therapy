...
class SafetyClassifier:
    ...
    def classify(self, text: str) -> SafetyClassification:
        ...
        FAIL-CLOSED DESIGN: Any API error or unparseable response defaults to
        'elevated', never 'routine'. An ambiguous failure should never silently
        look like "all clear" — the Memory Agent and Router still provide a
        second safety net, and the Mood Score override guard in the Router will
        downgrade to 'routine' if the user's self-reported score is clearly positive.
        """
        prompt = f"""
        ...
        try:
            response = self.client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    ...
                    temperature=0.0,  # Deterministic — safety calls should not vary
                ),
            )
            ...
            # Validate tier value — reject anything unexpected
            if result.tier not in ("routine", "elevated", "crisis"):
                print(f"[SafetyClassifier] WARNING: Invalid tier '{result.tier}', failing closed to 'elevated'.")
                result.tier = "elevated"

            return result

        except Exception as e:
            # FAIL CLOSED: default to 'elevated' on any error.
            #
            # Why not 'routine'? Because the cost of a false negative on a safety
            # call (someone in crisis gets no resources) is far higher than the cost
            # of a false positive (someone having a good day sees a gentle "you're
            # not alone" message). The Router's Mood Score override guard further
            # mitigates false positives: if the user's self-reported score is >= 7,
            # the router will downgrade 'elevated' back to 'routine' anyway.
            print(f"[SafetyClassifier] ERROR during classification: {e}. Failing closed to 'elevated'.")
            return SafetyClassification(
                tier="elevated",
                confidence=0.0,
                reasoning=f"Fallback triggered due to API error: {str(e)}. Failing closed to elevated."
            )
